...
const Header = () => {
  const navItems = [
    ...
    { id: 3, name: "Вхід", link: "/authorization" },
    ...
    <>
      <header className="header container-fluid" id="header_fixed">
        ...
        <div className="navbar-brand col-lg-5 col-md-6 col-9">
          <Link
            to="/"
            className="header__logo container row"
            onClick={() => setIsOpen(false)}
          >
          ...
          <ul className={isOpen ? "open" : ""}>
            {navItems.map((e) => (
              <li className="nav_item">
                <Link
                  to={`${e.link}`}
                  className="header__link nav-link navbar-brand row"
                  onClick={() => setIsOpen(false)}
                >
                  {e.name}
                </Link>
